# -*- coding: utf-8 -*-
"""
Created on Mon Jul 14 20:08:12 2025
...
def introduce_manual_noise(answer, degrade=False):
    harmless_noise = ["umm...", "uhh...", "I guess", "you know", "like", "I think", "sort of", "kinda"]
    wrong_responses = [
...
for i in range(500):
    ...
    for skill in skills_used:
        ...
        if skill == "SQL" and answer_flags.get("What is SQL?") == "not answered":
            unsupported_skills += 1
        if skill == "Python" and answer_flags.get("Tell me about yourself.") == "not answered":
            unsupported_skills += 1

    # FINAL DECISION — ONLY "Hire" or "Reject"
    hiring_decision = "Hire" if overall_score >= 4.0 and bad_answer_count <= 1 and unsupported_skills == 0 else "Reject"

    feedback = (
        "Excellent candidate. Recommended." if hiring_decision == "Hire"
        else "Not suitable currently. Needs stronger support for mentioned skills."
    )

    summary_issues = []
    if "Power BI" in skills_used and answer_flags.get("What is your experience with Power BI?") == "not answered":
        summary_issues.append("Power BI listed but not confidently answered")
    if "SQL" in skills_used and answer_flags.get("What is SQL?") == "not answered":
        summary_issues.append("SQL listed but not confidently answered")
    if "Python" in skills_used and answer_flags.get("Tell me about yourself.") == "not answered":
        summary_issues.append("Python listed but not confidently answered")
